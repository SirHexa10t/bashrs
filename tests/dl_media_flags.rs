...
//! |------------------------------------|--------------------|---------------------|------------|-------------|------------------|
//! | `dl <vid>`                         | dl vid + dl subs   | —                   | —          | —           | —                |
//! | `dl <vid> --thumbnail`             | … + dl thumb       | dl thumb            | dl thumb   | —           | —                |
//! | `dl <vid> --subtitles`             | dl vid + dl subs   | dl subs             | —          | dl subs     | —                |
//! | `dl <vid> --thumbnail --subtitles` | … + dl thumb       | dl subs + dl thumb  | dl thumb   | dl subs     | —                |
//!
...
//!
//! These tests download from YouTube for real and drive the bundled yt-dlp/ffmpeg/deno, so they
//! are `#[ignore]`d out of the default suite. Run them with:
//!
//! ```text
//! cargo test --test dl_media_flags -- --ignored --test-threads=1
//! ```
...
fn scratch(tag: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("bashrs_dl_media_{tag}_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    dir
}

/// Run `dl <URL> --into <dir> <flags…>` through the real binary, asserting success; returns the
/// combined output (status lines carry ANSI colour, but the message text stays contiguous, so
/// plain `contains` matches). Always `--no-cookies`: the sample is public, and a test must never
/// lean on — or transmit — whatever private cookie store the running machine has imported.
fn dl(dir: &Path, flags: &[&str]) -> String {
    let out = Command::new(env!("CARGO_BIN_EXE_bashrs"))
        .args(["dl", URL, "--no-cookies", "--into"])
        .arg(dir)
        .args(flags)
        .output()
    ...
    assert!(out.status.success(), "dl {flags:?} failed:\n{text}");
    text
}
