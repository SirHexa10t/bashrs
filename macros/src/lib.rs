...
/// - `#[shell_body("…")]` — the command is a pure shell function with this body instead of a
...
fn expand(args: CategoryArgs, module: ItemMod) -> syn::Result<TokenStream2> {
    ...
    let mut shell_fns: Vec<(String, String, String)> = Vec::new();
    ...
    for item in items {
        ...
        // A `#[shell_body]` command lives entirely in the sourcefile — one shell name, no Args
        // struct, no clap variant or dispatch. The Rust function is only its declaration site
        // (name + doc), kept for rustdoc but never called.
        if let Some(body) = shell_body {
            if !func.sig.inputs.is_empty() {
                return Err(syn::Error::new_spanned(
                    &func.sig,
                    "a #[shell_body] command runs entirely in the shell — it takes no arguments",
                ));
            }
            if !visible_aliases.is_empty() || after.is_some() || piped.is_some() || trailing_newline {
                return Err(syn::Error::new_spanned(
                    &func.sig,
                    "#[shell_body] supports a single plain shell name — drop \
                     #[alias]/#[prefixed]+#[unprefixed]/#[after]/#[piped]/#[trailing_newline]",
                ));
            }
            shell_fns.push((name, body, doc_summary(&docs)));
            kept.push(quote!(#[allow(dead_code)] #func));
            continue;
        }
    ...
    let shell_fn_rows = shell_fns.iter().map(|(name, body, comment)| quote!((#name, #body, #comment),));
    ...
    Ok(quote! {
        ...
        impl #command {
            ...
            /// The category's pure-shell commands (`#[shell_body]`): `(name, body, comment)`,
            /// emitted into the sourcefile as plain functions instead of binary wrappers.
            pub fn shell_functions() -> &'static [(&'static str, &'static str, &'static str)] {
                &[#(#shell_fn_rows)*]
            }
        }
    })
...
/// The first `#[doc]` line of a command — its one-line summary, as the sourcefile comment.
fn doc_summary(docs: &[Attribute]) -> String {
    docs.iter()
...
struct CommandAttrs {
    ...
    /// `#[shell_body("…")]` — a pure shell function with this body, instead of a binary call.
    shell_body: Option<String>,
}
...
fn take_command_attrs(attrs: &mut Vec<Attribute>) -> syn::Result<CommandAttrs> {
    ...
    for attr in std::mem::take(attrs) {
        ...
        } else if attr.path().is_ident("shell_body") {
            parsed.shell_body = Some(attr.parse_args::<syn::LitStr>()?.value());
        } else {
            kept.push(attr);
        }
    }
