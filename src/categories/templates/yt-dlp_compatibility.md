...
2. **XFF spoofing doesn't work on YouTube.** `_GEO_BYPASS = False` is hardcoded in the YouTube extractor, which only disables yt-dlp's *automatic* geo-bypass — an explicit `--xff`/`--geo-bypass-country` still *sends* an `X-Forwarded-For` header. YouTube just ignores it, enforcing geo by the real connection IP (issue #9883 shows `--xff` failing on a live block). Maintainer (seproDev) on #9883: *"XFF only works for very few sites. Youtube is not one of them. You either need to use a system wide VPN or specify a proxy server."* Use a real IP via OS-level VPN or `--proxy`.
