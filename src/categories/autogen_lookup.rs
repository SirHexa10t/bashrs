...
mod commands {
    ...
    #[unprefixed]
    pub fn g(args: GrepArgs) { _grep(&args.pattern, 0, args.source.as_deref()); }

    /// Case-insensitive literal search, showing 2 lines of context around each match
    #[unprefixed]
    pub fn g2(args: GrepArgs) { _grep(&args.pattern, 2, args.source.as_deref()); }

    /// Case-insensitive literal search, showing 3 lines of context around each match
    #[unprefixed]
    pub fn g3(args: GrepArgs) { _grep(&args.pattern, 3, args.source.as_deref()); }

    /// Case-insensitive literal search, showing 5 lines of context around each match
    #[unprefixed]
    pub fn g5(args: GrepArgs) { _grep(&args.pattern, 5, args.source.as_deref()); }

    /// Case-insensitive literal search, showing 8 lines of context around each match
    #[unprefixed]
    pub fn g8(args: GrepArgs) { _grep(&args.pattern, 8, args.source.as_deref()); }

    /// Case-insensitive literal search, showing 25 lines of context around each match
    #[unprefixed]
    pub fn g25(args: GrepArgs) { _grep(&args.pattern, 25, args.source.as_deref()); }
    ...
    /// `context` lines around each match (0 = none), using the `grep` crate in-process. Matches
    /// are coloured only when writing to a terminal, like `--color=auto`.
    fn _grep(pattern: &str, context: u32, source: Option<&str>) {
        let text = match input::read_input(source) {
            Ok(text) => text,
            Err(err) => return eprintln!("g: {err}"),
        };
        streamgrep::filter(pattern, &text, context as usize);
    }
}
