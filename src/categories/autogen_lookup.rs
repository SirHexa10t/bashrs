...
mod grep_commands {
    ...
    /// Case-insensitive grep-search (literal, or regex with -E), colouring matches
    #[unprefixed]
    pub fn g(args: GrepArgs) { _grep(&args); }

    /// Case-insensitive grep-search, showing 2 lines of context around each match
    #[unprefixed]
    pub fn g2(args: GrepBase) { _grep(&GrepArgs { base: args, context: 2 }); }

    /// Case-insensitive grep-search, showing 3 lines of context around each match
    #[unprefixed]
    pub fn g3(args: GrepBase) { _grep(&GrepArgs { base: args, context: 3 }); }

    /// Case-insensitive grep-search, showing 5 lines of context around each match
    #[unprefixed]
    pub fn g5(args: GrepBase) { _grep(&GrepArgs { base: args, context: 5 }); }

    /// Case-insensitive grep-search, showing 8 lines of context around each match
    #[unprefixed]
    pub fn g8(args: GrepBase) { _grep(&GrepArgs { base: args, context: 8 }); }

    /// Case-insensitive grep-search, showing 25 lines of context around each match
    #[unprefixed]
...
mod tree_commands {
    ...
    /// Roughly `find`+`grep`: Recursively search a directory for expression(s) — filenames, then file contents
    #[unprefixed]
    #[trailing_newline]
