...
pub(super) fn default_picks() -> Vec<Pick> {
    ["en", "en-US", "en-GB"]
...
pub(super) fn video_picks(url: &str, env: Env) -> (Option<String>, Vec<Pick>) {
...
pub(super) fn batch_probe(url: &str, indexes: &[String], env: Env) -> Vec<Planned> {
    let mut argv = seeded(env);
...
pub(super) fn group_by_langs(planned: &[Planned]) -> Vec<Vec<&Planned>> {
