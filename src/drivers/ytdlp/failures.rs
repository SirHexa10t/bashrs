...
use super::invocation::{ARCHIVE_NAME, ytdlp_invocation};
...
pub(super) fn capture_ytdlp(argv: Vec<OsString>) -> Option<(bool, String, String)> {
...
pub(super) fn diagnose_failure(base: Vec<OsString>, label: &str, rescue: GeoRescue) -> Option<String> {
...
pub(super) fn dead(line: String) -> String {
    eprintln!("dl: {line}");
    line
}
...
/// by hand.
pub(super) fn scrub_ledger(dir: &Path) {
...
pub(super) fn write_ledger(dir: &Path, lines: &[String]) {
    if lines.is_empty() {
        return;
    }
    let path = dir.join(FAILED_LEDGER);
