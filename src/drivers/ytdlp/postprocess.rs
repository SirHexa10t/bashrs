//! Patch already-downloaded files: cover art (thumbnails), embedded subtitle tracks, and
//! audio subtitle tags — idempotent ffmpeg/lofty passes over what yt-dlp wrote, safe to
//! re-run across a whole collection.

use std::ffi::OsString;
use std::path::{Path, PathBuf};

use crate::support::doc_style;
use crate::support::exec::{capture_output, capture_stdout};
use super::invocation::{Env, seeded, ytdlp_invocation};
use super::scan::{ScanEntry};
use super::subtitles::{Pick, batch_probe};
...
pub(super) fn mark_auto_titles(root: &Path, id: &str, picks: &[Pick], env: Env) {
...
/// are must-try.
pub(super) fn embed_subtitle_tags(root: &Path, id: &str, picks: &[Pick]) {
    if picks.is_empty() {
        return;
    }
    let Some(file) = find_by_id(root, id) else { return };
...
pub(super) fn finish_media(root: &Path, id: &str, picks: &[Pick], env: Env) {
    if env.audio {
...
pub(super) fn embed_thumbnails(dir: &Path, ids: &[String], env: Env) {
    if ids.is_empty() {
        return;
    }
...
pub(super) fn embed_subtitles(into: &Path, id: &str, url: &str, expected: &[Pick], env: Env) {
...
pub(super) fn patch_collection_subtitles(url: &str, entries: &[&ScanEntry], home: &Path, env: Env) {
    if entries.is_empty() {
        return;
    }
...
pub(super) fn find_by_id(root: &Path, id: &str) -> Option<PathBuf> {
...
mod tests {
    use super::*;
    use crate::drivers::ytdlp::invocation::{Env, bundled_ffmpeg_dir};
    use crate::drivers::ytdlp::subtitles::{Pick};
    use crate::drivers::ytdlp::testutil::{ff_bin, ffmpeg_or_skip, scratch_dir};
